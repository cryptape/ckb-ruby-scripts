...
# This contract might also need 2 optional unsigned argument:
# 2. (optional) supermode signature, when present and verified, the transaction
# can perform super mode operations
# 3. (optional) exit action, used unlock the locked tokens.
...
if ARGV.length == 3
  ...
  tx["inputs"].each_with_index do |input, i|
    if CKB.load_script_hash(i, CKB::Source::INPUT, CKB::Category::TYPE) == contract_type_hash
      blake2b.update(CKB::CellField.new(CKB::Source::INPUT, i, CKB::CellField::DATA).read(0, 16))
    end
  end
  tx["outputs"].each_with_index do |output, i|
    ...
    if CKB.load_script_hash(i, CKB::Source::OUTPUT, CKB::Category::TYPE) == contract_type_hash
      blake2b.update(CKB::CellField.new(CKB::Source::OUTPUT, i, CKB::CellField::DATA).read(0, 16))
    end
  end
...
if ARGV.length != 4
  ...
  output_sum = tx["outputs"].size.times.map do |i|
    if CKB.load_script_hash(i, CKB::Source::OUTPUT, CKB::Category::TYPE) == contract_type_hash
      CKB::CellField.new(CKB::Source::OUTPUT, i, CKB::CellField::DATA).read(0, 16).unpack("Q<")[0]
    else
      0
    end
  end.reduce(&:+)
